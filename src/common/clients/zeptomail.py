...
UNSUPPORTED_ATTACHMENT_FORMATS = [
    "ade",
    "adp",
    "bat",
    "chm",
...
class Email:
    ...
    def send(
        ...
        payload = {
            ...
            "to": [
                {"email_address": {"address": email, "name": f"TO_{index}"}}
            ...
            [
                {
                    "content": content,
                    "mime_type": (
                        _check_mime_type(mime_type) or self.DEFAULT_MIME_TYPE
                    ).value,
                    "name": name,
...
def test_send_file(path, mime_type, email):
    ...
    attachments = [
        (
            bytes_to_base64(contents),
            MimeType(*reversed(mime_type.split("/", 2))),
